function [ fit ] = fit_GPBSS_AIC( X, Y, regr, corr, bs, lob, upb, theta0 )
...
bspline = bs(repmat(round(length(bs)/2),size(X,2),1));
Ut = bspline(1).func(X(:,1));
for i = 2 : size(X,2)
    Ui = bspline(i).func(X(:,i));    Ut = sparse_khatrirao(Ut, Ui);
end
F = regr(X);
data = struct('corr',corr, 'regr',regr, 'bs',bspline, 'X',X, 'Y',Y, 'F',F, ...
...
if isempty(lob) && isempty(upb) % without optimization
    ...
    for p = 1 : size(X,2)  % knots selection
        AIC = [];
        for m = 1 : length(bs)
            bspline(p) = bs(m);
            Ut = bspline(1).func(X(:,1));
            ...
            data = struct('corr',corr, 'regr',regr, 'bs',bspline, 'X',X, 'Y',Y, 'F',F, ...
                'Fu',Ut*F, 'Yu',Ut*Y, 'Ut',Ut, 'UtU',Ut*Ut');
            obj = obj_func(theta, data); % [~, obj, fit, ~] = boxmin(@obj_func, theta, lob, upb, data);
            AIC = [AIC; 2*prod([bspline(:).p])+obj];
        end
        AIC(isinf(AIC)) = nan; % figure; plot(AIC);
        [~, index] = min(AIC); bspline(p) = bs(index);
    end
    Ut = bspline(1).func(X(:,1));
    for i = 2 : size(X,2)
        Ui = bspline(i).func(X(:,i));    Ut = sparse_khatrirao(Ut, Ui);
    ...
    data = struct('corr',corr, 'regr',regr, 'bs',bspline, 'X',X, 'Y',Y, 'F',F, ...
        'Fu',Ut*F, 'Yu',Ut*Y, 'Ut',Ut, 'UtU',Ut*Ut');
    [theta, obj, fit, ~] = boxmin(@obj_func, theta, lob, upb, data);Nk = [bspline(:).p];
end
m = prod([bspline(:).p]);   n = size(X,1);
if m >= n
    warning(['Number of control points m = ' num2str(m) ...
             ' exceeds the number of data samples n = ' num2str(n) ...
             '. Please consider reducing the number of control points to maintain a favorable trade-off ']);
end
