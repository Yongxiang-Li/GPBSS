...
OjbFun = @Fun_Hills;    
ACFun = @Sampling_BLUP;
p = 2;   maxIter = 40;  s = 20;   numExperiments = 10;
lb_GPBSS = [0.1*ones(1,p), 0.01];   ub_GPBSS = [30*ones(1,p), 1];   theta0_GPBSS = (lb_GPBSS + ub_GPBSS) / 2;
fitGP = @(X,Y,theta) fit_GPBSS(X, Y, @regpoly1, @corr_gauss, init_Bspline(3, [25 25]), lb_GPBSS, ub_GPBSS, theta);
...
for expIdx = 1:numExperiments
    ...
    expStartTime = tic; 
    dmodelStartTime = tic;    
    dmodel0 = fitGP(X0, Y0, theta0_GPBSS);   
    dmodel0.time = toc(dmodelStartTime);
    [design, dmodels] = BO(dmodel0, OjbFun, fitGP, @predict_GPBSS, ACFun, [], s, maxIter);
...
save(['result_GPBSSSO_batch.mat'], 'results_GPBSS', 'iterTimes_GPBSS', 'expTimes_GPBSS');
maxIter = 40;
iterations = 1:maxIter+1; iterations = iterations(:);
mean_GPBSS = mean(results_GPBSS, 2);
std_GPBSS = std(results_GPBSS, 0, 2);
color_gpbss = [0.85, 0.33, 0.1];
fontname = 'Times New Roman';  fontsize = 10;
figure('Units','centimeters','Position',[0,0,8,5.5]); hold on;
fill([iterations; flipud(iterations)], ...
     [mean_GPBSS + std_GPBSS; flipud(mean_GPBSS - std_GPBSS)], ...
     color_gpbss, 'FaceAlpha', 0.2, 'EdgeColor', 'none');
h_gpbss = plot(iterations, mean_GPBSS, '-', ...
               'LineWidth', 1.2, 'Color', color_gpbss);
