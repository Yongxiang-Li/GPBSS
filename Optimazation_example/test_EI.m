...
%% ---------- GPBSS 模型 ----------
lb_GPBSS = [0.1*ones(1,p), 0.01];   ub_GPBSS = [50*ones(1,p), 1];   theta0_GPBSS = (lb_GPBSS + ub_GPBSS) / 2;
fitGP_GPBSS = @(X,Y,theta)fit_GPBSS(X, Y, @regpoly1, @corr_gauss, init_Bspline(4, [30 30]), lb_GPBSS, ub_GPBSS,theta0_GPBSS);
...
for expIdx = 1:numExperiments
    ...
    expStartTime = tic;
    dmodelStartTime = tic;
    dmodel0 = fitGP_GPBSS(X0, Y0, theta0_GPBSS);
...
save(['result_GPBSSSO_EI' datestr(today, 'mmdd') '.mat'], 'results_GPBSS', 'iterTimes_GPBSS', 'expTimes_GPBSS');
